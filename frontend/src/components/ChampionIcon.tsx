...
export function ChampionIcon({
  ...
  const playerChampionStats = playerStats
    ? getChampionStats(playerStats, championId)
    : null;
  ...
  const tooltipContent = (
    <div className="space-y-2">
      ...
      {playerChampionStats && (
        <div>
          <div className="mb-1 text-xs text-gray-400">Your Performance:</div>
          <div
            className={`text-sm ${playerChampionStats.winRate < 0.5 ? "text-red-400" : "text-green-400"}`}
          >
            {playerChampionStats.wins}-{playerChampionStats.losses} (
            {(playerChampionStats.winRate * 100).toFixed(0)}% WR)
            <span className="text-gray-300">
              {" "}
              in {playerChampionStats.games} games
            </span>
          </div>
        </div>
      )}
      ...
      {championData.roles && championData.roles.length > 0 && (
        <div>
          <div className="mb-1 text-xs text-gray-400">Roles:</div>
          <div className="flex flex-wrap gap-1">
            {championData.roles.map((role) => (
              <span
                key={role}
                className="inline-block rounded-full border border-gray-600 bg-gray-700 px-2 py-0.5 text-xs text-gray-200"
              >
                {formatTag(role)}
              </span>
            ))}
      ...
      {championData.damageTypes && championData.damageTypes.length > 0 && (
        <div>
          <div className="mb-1 text-xs text-gray-400">Damage:</div>
          <div className="flex flex-wrap gap-1">
            {championData.damageTypes.map((damageType) => (
              <span
                key={damageType}
                className={`inline-block rounded-full border px-2 py-0.5 text-xs ${getDamageTypeColor(damageType)}`}
              >
      ...
      {championData.tags && championData.tags.length > 0 && (
        <div>
          <div className="mb-1 text-xs text-gray-400">Tags:</div>
          <div className="flex flex-wrap gap-1">
  ...
  return (
    <TooltipProvider>
      <Tooltip delayDuration={0}>
        <TooltipTrigger asChild>{championImage}</TooltipTrigger>
